...
import { Heart, MessageCircle, Share, Bookmark, MoreHorizontal, Flag, Send, Eye, AlertCircle } from 'lucide-react';
...
export const FeedPost = ({ post, onReport }: FeedPostProps) => {
  ...
  const [comments, setComments] = useState<Comment[]>([
    {
      id: '1',
      user: { name: 'Alice Johnson', username: 'alice', avatar: '/placeholder.svg' },
    ...
    {
      id: '2',
      user: { name: 'Bob Smith', username: 'bob', avatar: '/placeholder.svg' },
      content: 'Love this!',
      timestamp: '30m ago',
      likes: 3
    }
  ...
  const handleSave = () => {
    ...
    toast({
      ...
      description: saved ? 'Post removed from saved items' : 'Post added to saved items',
    });
  };
        ...
        {post.type === 'psa' && (
          <div className="mb-3 p-3 bg-orange-100 border border-orange-200 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm font-medium text-orange-800">Public Service Announcement</span>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex space-x-2">
                <Input
                  ...
                  className={`flex-1 ${commentError ? 'border-red-500 focus:border-red-500' : ''}`}
                  maxLength={500}
                />
                <Button 
